...
class sly_ChannelAttentionBlock(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        super(sly_ChannelAttentionBlock, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
...
class FftLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        center_h, center_w = x.shape[-2] // 2, x.shape[-1] // 2
        low_freq = x[..., :center_h, :center_w]
        high_freq = x[..., center_h:, center_w:]
        # Apply channel attention to high-frequency information
        high_freq = sly_ChannelAttentionBlock(self.in_channels)(high_freq)
        # Merge low-frequency and high-frequency information
        x = torch.cat([low_freq, high_freq], dim=(-2, -1))
        # Shift DC component back to corner
        x = torch.fft.ifftshift(x, dim=(-2, -1))
        # IFFT
        x = ifft2(x, norm='ortho')
        return x


# Example usage
class Sly_FFT(nn.Module):
    def __init__(self):
        super(Sly_FFT, self).__init__()
